    ...
    if (aspan <= 32)
        {
            if (n_reductions < 24)  // small matrix special case
                ...
                {
                    reduce_kernel<T, ReduceOp, 32><<<n_outputs, 32>>>(
                        A, result, op, n_reductions, aspan, 0, false, identity, pProcess);
                }
        }
    else if (aspan <= 64)
        {
            reduce_kernel<T, ReduceOp, 64><<<n_outputs, 64>>>(A, result, op, n_reductions, aspan, 0,
                                                              false, identity, pProcess);
        }
    else if (aspan <= 128)
        {
            reduce_kernel<T, ReduceOp, 128><<<n_outputs, 128>>>(A, result, op, n_reductions, aspan,
                                                                0, false, identity, pProcess);
        }
    else if (aspan <= 256)
        {
            reduce_kernel<T, ReduceOp, 256><<<n_outputs, 256>>>(A, result, op, n_reductions, aspan,
                                                                0, false, identity, pProcess);
        }
    else if (aspan <= 512)
        {
            reduce_kernel<T, ReduceOp, 512><<<n_outputs, 512>>>(A, result, op, n_reductions, aspan,
                                                                0, false, identity, pProcess);
        }
    else if (aspan <= 1024)
        {
            reduce_kernel<T, ReduceOp, 1024><<<n_outputs, 1024>>>(
                A, result, op, n_reductions, aspan, 0, false, identity, pProcess);
        }
    else
        {
