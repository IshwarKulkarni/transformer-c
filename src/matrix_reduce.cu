...
this kernel is laid out in 4 dimensions: 1d block, 3d grid. softmax_out and gradient_in are same
...
reduced by softmax output_width: num_outputs, Number of column vectors produced by softmax forward
function.

Summation for the dot product is accomplished like the reduction operation in other kernels
in this file.
*/
                               ...
                               const Matrix<T> grad_in)
{
    ...
    auto w_extent = sm_out.template get_extent<WIDTH_IDX>(b);
    if (w_max > w_extent) w_max = w_extent;
    __syncthreads();
...
void reduce(Matrix<T>& result, const Matrix<T>& A, ReduceOp op, T identity, PostProcess pProcess)
{
    ...
    for (uint32 b = 0; b < result.batch(); b++)
    {
        if constexpr (dim == WIDTH_IDX)
        {
            result.template set_extent<WIDTH_IDX>(b, 1);
        }
        else if constexpr (dim == HEIGHT_IDX)
        {
            result.template set_extent<HEIGHT_IDX>(b, 1);
        }
    }
}
...
template void reduce<FloatT, 0u, Max<FloatT>, Identity<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Max<FloatT>, FloatT, Identity<FloatT>);
