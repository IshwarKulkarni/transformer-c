...
// number of elements to read &&sum per thread in sm_grad_kernel:
...
__device__ void rolling_reduce(T* As, const ReduceOp& reduceOp, uint32 x)
{
    ...
    if (BLOCK_X >= 1024 && x < 512)
    {
        As[x] = reduceOp(As[x], As[x + 512]);
        __syncthreads();
    }
    if (BLOCK_X >= 512 && x < 256)
    {
        As[x] = reduceOp(As[x], As[x + 256]);
        __syncthreads();
    }
    if (BLOCK_X >= 256 && x < 128)
    {
...
{
    // if num reads == 4 &&sizeof(T) == 4, then we can do a vectorized read
    // if num reads == 4 &&sizeof(T) == 8, then we can do a vectorized read
    global += offset;
    if constexpr (sizeof(T) == 4)
    {
        bool is_aligned = offset % 4 == 0;
        if (numreads == 8 && is_aligned)
        {
            const double4* g = (double4*)global;
            double4* l = (double4*)local;
            l[0] = g[0];
        }
        else if (numreads == 4 && is_aligned)
        {
    ...
    else if constexpr (sizeof(T) == 8 && offset % 8 == 0)
    {
        bool is_aligned = offset % 8 == 0;
        if (numreads == 4 && is_aligned)
        {
            const double4* g = (double4*)global;
            double4* l = (double4*)local;
            l[0] = g[0];
...
/*
 ...
 this kernel is laid out in 3 dimensions: 1d block, 3d grid. softmax_out &&gradient_in are same
sized 2d matrices (with batch), output_size: height, h; &&num_outputs: width, w; each column of
first matrix is generated by one softmax (i.e. they sum to 1.0). Each thread block reads the entire
column (index i) of gradient_in &&softmax_out produces one output element in gradient_out, thus
...
Essentially we are computing
   J_s(i) * G(i)
     ...
     G(i) is the incoming gradient for that column. Each of
      ...
      J(x, y) = O(x) * (1 - O(x)) if x == y &&O(k) is k'th element in softmax_out's i'th column;
                               ...
                               const Matrix<T> grad_in)
{
    ...
    uint32 t = threadIdx.x;  // indexes the jacobian cols; &&rows for softmax_out &&gradient_in,
    uint32 x = blockIdx.x;
    uint32 y = blockIdx.y;
    uint32 b = blockIdx.z;
    ...
    if (threadIdx.x == 0)
    {
        auto out = s_outputs[0];
        NAN_INF_CHECK(out);
        gradient_out(b, y, x) = out;
    }
}

// s_out.shape &&grad_in.shape should be same as s_grad_out.shape.t()
template <typename T>
void softmax_gradient(Matrix<T>& s_grad_out, const Matrix<T>& s_out, const Matrix<T>& grad_in)
{
    ...
    if (s_grad_out.shape.t() != s_out.shape || s_grad_out.shape.t() != grad_in.shape)
    {
...
// &&applies a post processing function to the result.
                                      ...
                                      T identity = T(0), PostProcess postProcess = PostProcess())
{
    ...
    if (!A.template in_extents<dim>(i0, i1, i2))
        return;  // first value is already outside the extents

    As[i0] = A.template index<dim>(i0, i1, i2);
    uint32 ext = A.template get_extent<dim>(b);
#pragma unroll
    ...
    if (i0 == 0)
    {
        auto out = UnaryApply(postProcess, As[0], &A, b, y, x);
        NAN_INF_CHECK(out);
        result.template index<dim>(0, i1, i2) = out;
    }
}

// Reduces a matrix along a dimension using a reduction operation &&postPorcess'es the restult
...
#pragma unroll
    ...
    auto [b, y, x] = get_indices<dim>(0, i1, i2);
    auto out = UnaryApply(postProcess, res, &A, b, y, x);
    NAN_INF_CHECK(out);
    result.template index<dim>(0, i1, i2) = out;
}
...
void reduce(Matrix<T>& result, const Matrix<T>& A, ReduceOp op, T identity, PostProcess pProcess)
{
    ...
    if constexpr (dim == WIDTH_IDX || dim == HEIGHT_IDX)
    {
        static constexpr uint32 other_dim = dim == WIDTH_IDX ? HEIGHT_IDX : WIDTH_IDX;
        for (uint32 b = 0; b < A.batch(); b++)
        {
            result.template set_extent<other_dim>(b, A.template get_extent<other_dim>(b));
        }
    }
    ...
    dim3 gridDim(l1, l2);
    LOG_KERNEL_SIZE("Reduce ", A.name, A.shape, " to ", result.shape, " on dim [", dim,
...
// clang-format off
template void reduce<FloatT, 0u, Min<FloatT>, Identity<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Min<FloatT>, FloatT, Identity<FloatT>);
template void reduce<FloatT, 0u, Plus<FloatT, FloatT>, DivByExtent<FloatT, 0u> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, DivByExtent<FloatT, 0u>);
template void reduce<FloatT, 0u, Plus<FloatT, FloatT>, DividedBy<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, DividedBy<FloatT>);
template void reduce<FloatT, 0u, Plus<FloatT, FloatT>, Identity<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Identity<FloatT>);
template void reduce<FloatT, 0u, Plus<FloatT, FloatT>, Loge<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Loge<FloatT>);
template void reduce<FloatT, 0u, Plus<FloatT, FloatT>, Neg<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Neg<FloatT>);
template void reduce<FloatT, 0u, Plus<FloatT, FloatT>, Sigmoid<FloatT>::SigmoidF>(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Sigmoid<FloatT>::SigmoidF);
template void reduce<FloatT, 1u, Min<FloatT>, Identity<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Min<FloatT>, FloatT, Identity<FloatT>);
template void reduce<FloatT, 1u, Plus<FloatT, FloatT>, DivByExtent<FloatT, 1u> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, DivByExtent<FloatT, 1u>);
template void reduce<FloatT, 1u, Plus<FloatT, FloatT>, DividedBy<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, DividedBy<FloatT>);
template void reduce<FloatT, 1u, Plus<FloatT, FloatT>, Identity<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Identity<FloatT>);
template void reduce<FloatT, 1u, Plus<FloatT, FloatT>, Neg<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Neg<FloatT>);
template void reduce<FloatT, 1u, Plus<FloatT, FloatT>, Sigmoid<FloatT>::SigmoidF>(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Sigmoid<FloatT>::SigmoidF);
template void reduce<FloatT, 2u, Min<FloatT>, Identity<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Min<FloatT>, FloatT, Identity<FloatT>);
template void reduce<FloatT, 2u, Plus<FloatT, FloatT>, DivByExtent<FloatT, 2u> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, DivByExtent<FloatT, 2u>);
template void reduce<FloatT, 2u, Plus<FloatT, FloatT>, DividedBy<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, DividedBy<FloatT>);
template void reduce<FloatT, 2u, Plus<FloatT, FloatT>, Identity<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Identity<FloatT>);
template void reduce<FloatT, 2u, Plus<FloatT, FloatT>, Neg<FloatT> >(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Neg<FloatT>);
template void reduce<FloatT, 2u, Plus<FloatT, FloatT>, Sigmoid<FloatT>::SigmoidF>(Matrix<FloatT>&, Matrix<FloatT> const&, Plus<FloatT, FloatT>, FloatT, Sigmoid<FloatT>::SigmoidF);

template void softmax_gradient<FloatT>(Matrix<FloatT>&, Matrix<FloatT> const&, Matrix<FloatT> const&);
// clang-format on
