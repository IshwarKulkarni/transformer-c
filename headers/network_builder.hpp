...
using NodeCreatorMap = std::map<std::string, NodeCreatorFunc>;
...
struct NetworkBuilder
{
    ...
    */
    ...
    NodePtrMap m_nodes;
    NodeCreatorMap m_node_creators;
    ...
    string_vector read_params(const string_vector& param_names, std::istream& is,
    ...
    T get_value(const std::string& param_value)
    {
        ...
        if (param_value.find("->") != std::string::npos)
        {
            ...
            if (it == m_indirect_literals.end())
            {
                ...
                {
                    throw_rte_with_backtrace("Node `", node_name, "`, used in indirect literal `", param_value, "` it needs to be textually defined before being used in an indirect literal");
                }
                else
                {
