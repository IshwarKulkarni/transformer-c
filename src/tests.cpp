            ...
            for (uint32 x = 0; x < D.width; x++)
                {
                    ...
                    if (std::abs(c - d) > eps)
                        {
                            diff << y << ", " << x << " :\t" << std::setprecision(6)
                                 << std::setfill(' ') << std::setw(10) << c << ",\t"
                                 << std::setprecision(6) << std::setfill(' ') << std::setw(10) << d
...
int main()
{
    ...
    auto A = Matrix<FloatT>(3, 1, values);
    MatrixT C = MatrixT(1, 1);
    MatrixT D = MatrixT(1, 1);
    // reduce_meanCPU(C, A);
    reduce_mean(D, A);

    // std::cout << "C: " << *C.begin() << std::endl;
    std::cout << "D: " << *D.begin() << std::endl;
    return 0;
}
