...
FloatT add_noise_normal(Vec300& vec, FloatT mean, FloatT std, uint32* seed)
{
    ...
    norm = std::sqrt(norm);
    for (uint32 i = 0; i < N; i++) vec[i] /= norm;
    return cos_sim(orig, vec);
}

std::array<std::pair<float32, bool>, N> diff(const Vec300& a, const Vec300& b, float32 eps,
                                             uint32* count)
{
    std::array<std::pair<float32, bool>, N> diffs;
    for (uint32 i = 0; i < N; i++)
    {
...
static void write_binary(const std::string& filename, const std::vector<WordVecPair>& wordVecPairs)
{
    ...
    for (const WordVecPair& pair : wordVecPairs)
    {
        ...
        file.write((char*)&pair.second[0], N * sizeof(float32));
    }
    delete[] word_str;
}
...
static bool read_binary(const std::string& filename, std::vector<WordVecPair>& wordVecPairs)
{
    ...
    for (uint32 i = 0; i < n_words; i++)
    {
        ...
        file.read((char*)&vec[0], N * sizeof(float32));
...
Word2Vec::Word2Vec(const std::string& filename)
{
    std::vector<WordVecPair> wordVecPairs;
    read_text(filename, wordVecPairs);
