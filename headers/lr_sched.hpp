...
class LRScheduler
{
 public:
    ...
    virtual float32 update_lr(uint32 step)
    {
        (void)step;
        return m_current_lr;
    }
...
class StepLRScheduler : public LRScheduler
{
 public:
    ...
    virtual float32 update_lr(uint32 step) override
    {
...
inline LRScheduler* create_lr_scheduler(const VarArgs& args)
{
    ...
    if (lr_sched == "step")
        return new StepLRScheduler(args.get("lr", 0.001), args.get("min_lr", 1e-5),
                                   args.get("step_size", 1000), args.get("factor", 0.95));
