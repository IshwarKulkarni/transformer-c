...
├── src/           # Source files
├── static_data/   # Static data files, datasets etc.
└── tests/         # Test files
```
...
### Test Data Generation (`tests/datagen.py` and `tests/node_datagen.py`)

These scripts generate test data for various operations:

```bash
python tests/datagen.py <gen_command> <args>
python tests/node_datagen.py  # Generates all node test data
```

Available commands in datagen.py:
...
The node_datagen.py script generates test data for various neural network operations:
- Attention mechanism
- Linear layers
...
python tests/node_datagen.py  # Generate all node test data
```
